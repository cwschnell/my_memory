...
async def _process_text_recording_pipeline(
    ...
    if payload.type == "shopping":
        ...
        if not extracted_items:
            import re
            extracted_items = [i.strip().title() for i in re.split(r'[,\n]', transcript) if i.strip()]
    ...
    target_lodge_id = None
    lodge_str = payload.lodge_id or x_lodge_id
    if lodge_str and lodge_str.strip() and lodge_str.strip().lower() not in ("null", "none"):
        try:
            target_lodge_id = uuid.UUID(lodge_str.strip())
        except ValueError:
            target_lodge_id = None
